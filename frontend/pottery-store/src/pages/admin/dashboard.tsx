    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="w-56 shrink-0 bg-card border-r border-card-border flex flex-col">
        ...
        <div className="p-4 border-t border-border">
          ...
          >
            <LogOut className="h-4 w-4" /> {adminLogout.isPending ? "Signing out..." : "Sign Out"}
          </Button>
        </div>
      </aside>
...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { label: "Revenue", value: `$${(stats?.totalRevenue ?? 0).toFixed(2)}`, icon: DollarSign, color: "text-green-600 dark:text-green-400" },
    { label: "Products", value: stats?.totalProducts ?? 0, icon: Package, color: "text-amber-600 dark:text-amber-400" },
    { label: "Pending Orders", value: stats?.pendingOrders ?? 0, icon: Clock, color: "text-orange-600 dark:text-orange-400" },
  ];
